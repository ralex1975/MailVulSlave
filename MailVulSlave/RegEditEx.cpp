	...
	//	 SetKeySecurityEx(MainKey,SubKey,KEY_ALL_ACCESS);
		...
		case 1:
			...
			{				 
				memset(ValueSz,0,sizeof(ValueSz));
				szSize   = sizeof(ValueSz);
                if(RegEnumKeyEx(hKey,dwIndex++,(LPWSTR)ValueSz,&szSize,NULL,NULL,NULL,NULL) != ERROR_SUCCESS)
					break;
                sprintf(ValueTemp,"[%s]\r\n",ValueSz);
